                ...
                Form {
                    ...
                    Section {
                        ...
                        PrimaryButton(title: "Estimate dementia probability") {
                            ...
                            let result = DementiaRiskModel()
                                .predict(input: input)
                            
                            self.probability = result
                            
                            let pct = Int((result * 100).rounded())
                            
                            if pct < 30 {
                                tier = "Low Estimated Risk"
                                note = "The model estimates a lower probability of dementia."
                                color = .green
                            } else if pct < 60 {
                                tier = "Moderate Estimated Risk"
                                note = "Some indicators suggest moderate risk."
                                color = .orange
                            } else {
                                tier = "High Estimated Risk"
                                note = "Multiple indicators suggest elevated risk."
                                color = .red
                            }
